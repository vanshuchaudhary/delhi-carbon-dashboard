...
export default function SectorBreakdownPage() {
  ...
  useEffect(() => {
    getSectorBreakdown().then(setData);
  }, []);
  ...
  return (
    <div className="h-full w-full animate-fade-in flex flex-col gap-6 z-10 relative">
      <header className="mb-2">
        <h1 className="text-3xl font-bold text-slate-100">Sector Breakdown</h1>
        <p className="text-slate-400 mt-1">Analytics and historical carbon emissions by industry in Delhi</p>
      </header>

      {/* Sentinel Insight Box */}
      <div className="glass-panel p-5 border border-emerald-500/30 bg-emerald-500/5 rounded-2xl flex items-start gap-4 shadow-[0_0_20px_rgba(16,185,129,0.1)] animate-in fade-in slide-in-from-top-4 duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1 max-h-[70vh]">
        <div className="glass-panel p-6 border border-slate-800/50 flex flex-col shadow-xl">
          <h3 className="text-lg font-semibold text-slate-200 mb-6 flex items-center gap-2">
            <span className="w-1.5 h-6 bg-emerald-500 rounded-full"></span>
            Emissions by Sector (CO2 eq)
          </h3>
          <div className="flex-1 min-h-[300px]" style={{ color: '#94a3b8' }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
                <XAxis dataKey="sector" stroke="#64748b" tick={{fill: '#94a3b8'}} axisLine={false} tickLine={false} />
                ...
                />
                <Bar dataKey="co2_level" radius={[6, 6, 0, 0]}>
